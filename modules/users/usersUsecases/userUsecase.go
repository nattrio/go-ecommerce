...
func (u *usersUsecase) GetPassport(req *users.UserCredential) (*users.UserPassport, error) {
	...
	// Sign token
	accessToken, err := myshopauth.NewMyshopAuth(myshopauth.Access, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
	refreshToken, err := myshopauth.NewMyshopAuth(myshopauth.Refresh, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
...
func (u *usersUsecase) RefreshPassport(req *users.UserRefreshCredential) (*users.UserPassport, error) {
	// Parse token
	claims, err := myshopauth.ParseToken(u.cfg.Jwt(), req.RefreshToken)
	if err != nil {
		return nil, err
	}
	...
	accessToken, err := myshopauth.NewMyshopAuth(
		myshopauth.Access,
		u.cfg.Jwt(),
		newClaims,
	)
	...
	refreshToken := myshopauth.RepeatToken(
		u.cfg.Jwt(),
		newClaims,
